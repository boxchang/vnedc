...
def process_type_sheet(df, request):
    try:
        for _, row in df.iterrows():
            ...
            Process_Type.objects.update_or_create(
                process_code=process_code,
                process_name=process_name,
                defaults={'process_tw': process_tw, 'process_cn': process_cn, 'process_vn': process_vn,
                          'show_order': show_order, 'update_at': timezone.now(), 'update_by': request.user}
            )
    except Exception as e:
        print(e)
...
def parameter_type_sheet(df, request):
    try:
        for _, row in df.iterrows():
            ...
            process_type = Process_Type.objects.filter(process_code=process_type_code).first()

            Parameter_Type.objects.update_or_create(
                param_type_code=param_type_code,
                process_type=process_type,

                defaults={'control_high_column': control_high_column, 'control_low_column': control_low_column,
                          'control_table': control_table, 'update_at': timezone.now(), 'update_by': request.user}
            )
    except Exception as e:
        print(e)
...
def param_define_sheet(df, request):
    try:
        for _, row in df.iterrows():
            ...
            plant = Plant.objects.filter(plant_code=plant_code).first()
            mach = Machine.objects.filter(mach_code=machine_code).first()
            process_type = Process_Type.objects.filter(process_code=process_type_code).first()
            obj, created = ParameterDefine.objects.update_or_create(
                plant=plant,
                mach=mach,
                process_type=process_type,
...
def lab_parameter_sheet(df, request):
    try:
        ...
        for _, row in df.iterrows():
            ...
            obj, created = Lab_Parameter_Control.objects.update_or_create(
                plant=plant,
                mach=mach,
                item_no=item_no,
                process_type=process_type,
            ...
            if created:
                Lab_Parameter_Control.objects.filter(id=obj.id).update(
                    create_at=timezone.now(),
