...
def general_status(request):
    ...
    #Take out the machine name and machine line and order them by name list
    #Take out 2 lasted value of speed of machine line, if both is None then alert problems
    sql1 = f"""
        WITH LatestRecords AS (
                SELECT 
                    MachineName,
            ...
            ORDER BY
                ...
                cdm.COUNTING_MACHINE;
    """
    #Take out the existing of workorder on selected date worksheet, if exist workorder value is 1, else is 0
    sql2 = f"""
            WITH Machine AS (
                SELECT 
                    Id AS MachineID,
                    Name AS MachineName
                FROM 
                    [PMGMES].[dbo].[PMG_DML_DataModelList] 
                WHERE 
                    DataModelTypeId = 'DMT000003'
                    AND Name LIKE '%{selected_machine}%'
            ),
            WorkOrderCheck AS (
                SELECT 
                    m.MachineID,
            ...
            ORDER BY 
                MachineName;"""
    #Check IPQC Runcards ID and --Weights-- values for each time measureing gloves
    sql3 = f"""
        WITH Machine AS (
            SELECT * 
            FROM [PMGMES].[dbo].[PMG_DML_DataModelList] dl 
        ...
        InspectionCheck AS (
            SELECT 
                ...
                woi.LineId AS LineId,
                rc.Id AS RuncardId,
                rc.InspectionDate AS InspectionDate,
                rc.Period AS Period,
                qc.InspectionValue AS Weight
            FROM 
                Machine dml
    ...
    LEFT JOIN 
        ...
        AND cd.CreationTime BETWEEN CONVERT(DATETIME, '{selected_date} 06:00:00', 120) 
                            AND CONVERT(DATETIME, DATEADD(SECOND, -1, CONVERT(DATETIME, DATEADD(DAY, 1, '{selected_date}') + ' 06:00:00', 120)), 120)
        AND (cd.Speed < 60 OR cd.Speed IS NULL)
    WHERE 
...
"""
    db = mes_database()
    machine_line_status_and_2lasted_values = db.select_sql_dict(sql1)
    work_order_mode_list = db.select_sql_dict(sql2)
    run_card_mode_and_weights = db.select_sql_dict(sql3)
    machine_line_status_list = [[value['COUNTING_MACHINE'], value['Check']] for value in machine_line_status_and_2lasted_values]
    machine_line_lists = [value['COUNTING_MACHINE'] for value in machine_line_status_and_2lasted_values]
    group_machine_name_with_line = {}
    group_2lasted_speed_with_line = {}
    for index, machine_line in enumerate(machine_line_lists):
        machine_name = f"{machine_line.split('_CountingMachine_')[0]} {machine_line.split('_CountingMachine_')[-1][:-1]}"
        if machine_name not in group_machine_name_with_line:
            group_machine_name_with_line[machine_name] = []
            group_2lasted_speed_with_line[machine_name] = []
        group_machine_name_with_line[machine_name].append(f"{machine_line.split('_CountingMachine_')[0]} {machine_line.split('_CountingMachine_')[-1]}")
        group_2lasted_speed_with_line[machine_name].append(machine_line_status_list[index][1])
    machine_name_status = list(group_2lasted_speed_with_line.values())
    work_order_mode_lists = [value['WorkOrderMode'] for value in work_order_mode_list]  #[1, 1, 1, 1, 1, 1, 1, 0, 1, 1, 1, 1, 1, 1]
    runcards_machine_name_list = sorted(list(set([runcards['MachineName'] for runcards in run_card_mode_and_weights])),  key=lambda x: x[-2:])
    group_runcards_and_weights = {}
    for entry in run_card_mode_and_weights:
        machine_name = entry['MachineName']
        if machine_name not in group_runcards_and_weights:
            group_runcards_and_weights[machine_name] = {'MachineName': entry['MachineName'],'LineId': [], 'RuncardId': [], 'InspectionDate': [], 'Period': [], 'Weight': []}
        group_runcards_and_weights[machine_name]['LineId'].append(entry['LineId'])
        group_runcards_and_weights[machine_name]['RuncardId'].append(entry['RuncardId'])
        group_runcards_and_weights[machine_name]['InspectionDate'].append(entry['InspectionDate'])
        group_runcards_and_weights[machine_name]['Period'].append(entry['Period'])
        group_runcards_and_weights[machine_name]['Weight'].append(float(entry['Weight']) if entry['Weight'] is not None else None)
    group_runcards_and_weights_list = [[value['MachineName'],value['LineId'],value['RuncardId'],value['InspectionDate'],value['Period'],value['Weight']] for value in group_runcards_and_weights.values()]
    run_card_mode = []
    run_card_lost_details = []
    for machine_info in group_runcards_and_weights_list:
        temp = [value for value in period_times if value not in machine_info[-2]]
        run_card_mode.append(len(temp))
        run_card_lost_details.append(temp)

    speed_under_60_in_today = db.select_sql_dict(sql4)
    speed_under_60_in_today_list = {}
    for entry in speed_under_60_in_today:
        machine = entry['MachineName']
        line = entry['LINE']
        minute = entry['Minute']
        if machine not in speed_under_60_in_today_list:
            speed_under_60_in_today_list[machine] = {}
        if line not in speed_under_60_in_today_list[machine]:
            speed_under_60_in_today_list[machine][line] = 0
        if minute is not None:
            speed_under_60_in_today_list[machine][line] += 5
    today_speed_stop_list = [[count for line, count in line_counts.items()] for line_counts in speed_under_60_in_today_list.values()]

    today_machine_total_over60 = db.select_sql_dict(sql5)
    group_today_machine_total_over60  = {}
    for entry in today_machine_total_over60:
        machine_line = entry['MachineName']
        machine_line_id = entry['LINE']
        machine_line_period = entry['Period']
        if machine_line not in group_today_machine_total_over60:
            group_today_machine_total_over60[machine_line] = {}
        if machine_line_id not in group_today_machine_total_over60[machine_line]:
            group_today_machine_total_over60[machine_line][machine_line_id] = {}
        if machine_line_period not in  group_today_machine_total_over60[machine_line][machine_line_id]:
            group_today_machine_total_over60[machine_line][machine_line_id][machine_line_period] = []

        group_today_machine_total_over60[machine_line][machine_line_id][machine_line_period] .append(entry['Qty2'])
    today_machine_total_over60 = [
        [
                [sum(value for value in qtys if value is not None) for hour, qtys in hours.items()]
                for line, hours in lines.items()
        ]
        for machine, lines in group_today_machine_total_over60.items()
    ]

    today_machine_total_list = []
    for machine_data in today_machine_total_over60:
        today_machine_total_list.append(sum([sum(sublist) for sublist in machine_data]))

    machine_total_ok_ng = [[mtl] + [''] * len(tts) for mtl, tts in zip(today_machine_total_list, today_speed_stop_list)]
    run_cards_and_stop_time = [[rcm] + ss for rcm, ss in zip(run_card_mode, today_speed_stop_list)]
    work_order_and_machine_status = [[wom] + mns for wom, mns in zip(work_order_mode_lists, machine_name_status)]
    machine_name_with_line = [[key, [''] + value] for key, value in group_machine_name_with_line.items()] #[['NBR 1', ['NBR 1A', 'NBR 1B']]
    machine_name_with_line = [
    [sublist1[0], [[name, value, value2, value3] for name, value, value2, value3 in zip(sublist1[1], sublist2, sublist3, sublist4)]]
        for sublist1, sublist2, sublist3, sublist4  in zip(machine_name_with_line, work_order_and_machine_status,run_cards_and_stop_time,machine_total_ok_ng)
    ]
