...
def general_status(request):
    ...
    runcard_periods_with_time = [
        [machine_name, [[period, periods.get(period, []) if periods.get(period) != [None] else []] for period in period_times]]
        for machine_name, periods in group_runcard_machine.items()]   
    runcard_weights_with_time = [
        [machine_name,[[period, [float(value) for value in periods.get(period, []) if value is not None]]for period in period_times]]
    ...
    counting_data_on_period = [[[item[period] for item in counting_data if len(item) > period] 
            if not all(value in (None, 0) for value in [item[period] for item in counting_data if len(item) > period]) else []
    ...
    right_table_data = [
        [runcard_periods[i], weights_periods[i], counting_data_on_period[i], aoi_ok_mode[i][0], aoi_ng_mode[i][0], aoi_ok_mode[i][1], aoi_ng_mode[i][1]] 
        if len(aoi_ok_mode[i]) == 2 and len(aoi_ng_mode[i]) == 2
        else [runcard_periods[i], weights_periods[i], counting_data_on_period[i], aoi_ok_mode[i][0], aoi_ng_mode[i][0], aoi_ok_mode[i][1], aoi_ng_mode[i][1], aoi_ok_mode[i][2], aoi_ng_mode[i][2], aoi_ok_mode[i][3], aoi_ng_mode[i][3]]
