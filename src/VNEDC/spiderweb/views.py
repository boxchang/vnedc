...
def spiderweb(request):
    ...
    for name in monitor_list:
        ...
        sql = f"""
        select dt.id, dt.type_name, dl.status_id, dl.device_name, dlg.comment, 
        ...
        join  [VNEDC].[dbo].[spiderweb_monitor_device_list] dl
        on dl.device_type_id = dt.id
        join [VNEDC].[dbo].[spiderweb_monitor_device_log] dlg
        on dlg.func_name = dt.type_name
        where dl.status_id != 'S01' and enable = 'Y' and dlg.recover_msg is NULL and type_name = '{name}'
        group by dt.id, dt.type_name, dl.status_id, dl.device_name, dlg.comment
        """
        ...
        if str(name) == 'COUNTING DEVICE' or str(name) == 'AOI DEVICE':
            ...
            if str(name) == 'COUNTING DEVICE':
                ...
                monitor_msg.append([[item['device_name'], item['update_at'], item['comment']] for item in filtered_result])
